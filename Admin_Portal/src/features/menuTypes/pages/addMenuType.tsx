...
import {
    ...
    getMenuTypeByIdThunk,
} from "../menuTypesSlice";
import { AddMenuType, MenuType } from "../types";
import { toast } from "sonner";
import { Loader2 } from "lucide-react";

interface MenuTypeFormModalProps {
    isOpen: boolean;
    onClose: () => void;
...
const menuTypeSchema = yup.object().shape({
    name: yup
        .string()
        .required("Menu type name is required")
...
type MenuTypeFormData = yup.InferType<typeof menuTypeSchema>;

export const MenuTypeFormModal: React.FC<MenuTypeFormModalProps> = ({
    isOpen,
    onClose,
    menuType,
    mode,
    ...
    const { loading } = useSelector((state: RootState) => state.menuTypes);
    ...
    const {
        ...
        resolver: yupResolver(menuTypeSchema),
        defaultValues: {
            name: "",
            image: "",
            isActive: 0,
    ...
    useEffect(() => {
        if (mode === "edit" && id) {
            const fetchMenuTypeDetails = async () => {
                try {
                    const res = await dispatch(getMenuTypeByIdThunk(id)).unwrap();
                    ...
                    const errorMsg =
                        error instanceof Error
                            ...
                            : typeof error === "string"
                                ? error
                                : "Failed to fetch menu type details";
                    toast.error(errorMsg);
                }
            };
            fetchMenuTypeDetails();
        }
    ...
    const onSubmit = async (data: MenuTypeFormData) => {
        try {
            if (mode === "add") {
                const addData: AddMenuType = {
                    name: data.name,
                    image: image,
                };

                const result = await dispatch(createMenuTypeThunk(addData)).unwrap();
                if (result) {
                    toast.success(result.message);
                    await dispatch(getAllMenuTypesThunk({ page: 1, limit: 10 })).unwrap();
                    handleClose();
                }
            ...
            <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle className="text-2xl">
                        {mode === "add" ? "Create New Food Type" : "Edit Food Type"}
                    </DialogTitle>
                    <DialogDescription>
                        {mode === "add"
                            ? "Add a new food type with image"
                            : "Update the food type details"}
                    </DialogDescription>
                </DialogHeader>
